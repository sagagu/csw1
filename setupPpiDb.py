...
def createInterfaceCSV(pdbsToAnalyze):
    ...
    #        print('PDB,Chains,Chain,ResId,iCode,Symbol,Atom,MinDistance', file=distancePerAtom)
    #        print('PDB,Chain,Residue,ResId,iCode,Symbol,AtomType,ASA,tempFactor,Seperated', file=asaPerAtom)
            failedPDBs = []
...
def calcEnergyTerms(pdbsToAnalyze):
    ...
    #pdbsToAnalyze [['GHIJ', 'A:B'], ['ABCD', 'C:D']]
...
#        path = os.mkdir(os.path.join('results/cold_spots', '{}'.format(pdb)))
        chains2 = "".join(chains)
...
#                check_hetero=heteroatom.calculateVolume(pdb, interfaceAtoms)
...
#        final_unfavorable_cold_spots =[', '.join(map(str, x)) for x in f_cold_spot]
...
#        print('Identified cold spots in the interface due to CH interactions', file=cold_file)
    ...
    for pdbName, chains  in pdbsNamesToChains.items():
        if pdbName[0:4] not in pdbsNamesToChains: continue
#        path = os.mkdir(os.path.join('results/cold_spots', '{}'.format(pdbName)))
...
#            print('Identified charged residues which can act as cold spots due to SC interactions', file=same_int)
    ...
    with open(os.path.join(RESULTS_DIR, COLD_SPOTS_DIR, 'PyMOL_Representation.pml'),mode = 'w') as cold_file4:
        # with open(os.path.join(RESULTS_DIR, COLD_SPOTS_DIR, 'Cold spots residues due to CH interactions.csv')) as r1, open(os.path.join(RESULTS_DIR, COLD_SPOTS_DIR, 'Cold spots due to SC_interactions.csv')) as r2:
        #     line_file2 = r1.read()
        #     line_file3 = r2.read()
...
#        print(line_file2[0], file=cold_file4)
        try:
...
if __name__ == "__main__":
    try:
	    ...
	    # COLD_SPOTS_DIR = os.path.join(WORKING_DIRECTORY, RESULTS_DIR, 'parameters')
	    # PARAMETERS_DIR = os.path.join(WORKING_DIRECTORY, PARAMETERS_DIR, 'cold_spots')
	...
	#    e.g. [['GHIJ', 'A:B'], ['ABCD', 'C:D]]
        ...
        with open(os.path.join(RESULTS_DIR, COLD_SPOTS_DIR,'error.log'),'w') as error_file:
	        ...
	        if Any_error == "A 2-dimensional array must be passed.":
	            print('Please give a valid PDB ID and Chain identifiers for that PDB ID. Also check whether the chain Identifier form the ciorrect interface', file=error_file)
	            #print(Argument, file=my_file)
	        else:
